...
public:
decode_mac_impl(bool log, bool debug) : block("decode_mac",
...
int general_work (int noutput_items, gr_vector_int& ninput_items,
	...
	while(i < ninput_items[0]) {
		...
		if(tags.size()) {
			...
			if(frame.n_sym <= MAX_SYM && frame.n_encoded_bits <= MAX_BITS) {
				d_ofdm = ofdm;
				d_frame = frame;
				copied = 0;
...
void demodulate() {
	...
	cvec symbols;
	symbols.set_length(d_frame.n_sym * 48);
...
void decode_conv() {
	...
	dout << d_frame.n_encoded_bits << std::endl;

	vec rx_signal(deinter, d_frame.n_encoded_bits);

	code.reset();
	decoded_bits.set_length(d_frame.n_encoded_bits);

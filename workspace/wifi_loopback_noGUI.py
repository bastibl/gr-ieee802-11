...
def main(top_block_cls=wifi_loopback_noGUI, options=None):
    ...
    tb = top_block_cls(chan_est=options.chan_est, encoding=options.encoding, freq_offset=options.freq_offset, interval=options.interval, nmessages=options.nmessages, snr=options.snr)
    tb.start()
    tb.wait()
